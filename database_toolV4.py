...
from GUI_functions import (
    ...
    refresh_db_file_menu, pretty_print_sql
    )
from utils import (
...
##db_menu.add_command(label="Open Database...", command=lambda: menu_open_database(output_textbox))
...
##for filename in global_vars.recent_db_files:
##    db_menu.add_command(
##        label=f"Recent: {os.path.basename(filename)}",
##        command=lambda f=filename: choose_database(f, output_textbox=output_textbox)
##
##    )
...
##       # command=lambda f=filename: choose_database(f, output_textbox=output_textbox)
##        command=lambda f=filename: choose_database(f, output_textbox=output_textbox, window=window)
##
##    )
##for i, filename in enumerate(global_vars.recent_db_files, start=1):
##    db_menu.add_command(
##        label=f"{i}. {os.path.basename(filename)}",
##        command=lambda f=filename: choose_database(value=f, output_textbox=output_textbox, window=window)
##    )
...
##    db_menu.add_command(
##        label=f"{i}. {os.path.basename(filename)}",
##        command=lambda f=filename: (
##            global_vars.__setattr__('current_database', f),
##            output_textbox.config(state='normal'),
##            output_textbox.insert('end', f"Database selected: {os.path.basename(f)}\n"),
##            output_textbox.config(state='disabled'),
##            window.title(f"Python Database Tool – {f}")
##        )
##    )
...
Button(button_frame, text="Pretty Print", width=12, bg=bg_button, fg=text_colour,
